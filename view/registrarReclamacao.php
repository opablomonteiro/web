...
<body>
 ...
 <form  name="cadastro_reclamacao" method="POST" action="..\controller\controller.php">
        <label for="codpassr">Codigo da passagem:</label>
        <input name="codpassr" id="codpassr" required="required" type="text" placeholder="Digite codigo da passagem.."  maxlength="60"/>
    
        <label for="dataoco">Data da Ocorrencia:</label>
        <input name="dataoco" id="dataoco" required="required" type="date" placeholder="DD-MM-AAAA" />
     
        
 
        <label for="prob">Problema:</label>
        <textarea id = "txtprob"name="descricao" form="cadastro_reclamacao">Digete a reclamação...</textarea>
   
   
        <script>
            function alertReclamacaoReg() {
              alert("Reclamação registrada com sucesso!");
            }
            </script>
        <input type="hidden" name="acao" value="criar_reclamacao" id="acao">
        <input id = "btn-regrec"type="submit" type="button" class="button" value="Cadastrar" onclick="alertReclamacaoReg()"/>
